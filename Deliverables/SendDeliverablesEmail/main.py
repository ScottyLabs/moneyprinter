...
def getDeliverablesEmail (df):
    filtereddf = df[(df["Status"] == "Confirmed") & (df["Send New Email"] == "Yes")]
...
def sendDeliverables (request) :
    df = deliverables.get_as_df()
    recipientsList = getDeliverablesEmail(df)
